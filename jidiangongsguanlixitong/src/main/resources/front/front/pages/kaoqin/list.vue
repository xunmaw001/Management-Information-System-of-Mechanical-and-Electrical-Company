<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('kaoqin','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('kaoqin','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='kaoqinTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in kaoqinTypesList" :key="index" :class="kaoqinTypes===item.codeIndex?'tabActive':''"
                                  @tap="kaoqinTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.kaoqinName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('kaoqin','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                 <view v-show="item.kaoqinValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">考勤结果</text>
                     <text style="width: 70%">{{item.kaoqinValue}}</text>
                 </view>
                <view v-show="item.kaoqinContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">考勤详情</text>
                    <view style="width: 70%" v-html="item.kaoqinContent"></view>
                </view>
                <view v-show="item.kaoqinTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">考勤日期</text>
                    <text style="width: 70%">
                        <text v-if="item.kaoqinTime != null">{{item.kaoqinTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('kaoqin','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('kaoqin','审核') && item.kaoqinYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 考勤结果 字段 字典表查询方法
            async kaoqinTypesSelect(){
                let kaoqinTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'kaoqin_types',
                });
                kaoqinTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.kaoqinTypesList = kaoqinTypes.data.list
            },
            //当前表的 考勤结果字段类别搜索
			kaoqinTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.kaoqinTypes != 0) {
                    params.kaoqinTypes = this.kaoqinTypes
                }
                let _this = this
                let res = await this.$api.page(`kaoqin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kaoqin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`kaoqin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`kaoqin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.kaoqinContent!=null){
                        item.kaoqinContent = item.kaoqinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
