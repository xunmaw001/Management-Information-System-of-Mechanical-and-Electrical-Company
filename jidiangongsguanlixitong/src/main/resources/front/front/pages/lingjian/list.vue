	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"0px","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.lingjianName" type="text" placeholder="机电零件名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('lingjian','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('lingjian','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='lingjianTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in lingjianTypesList" :key="index" :class="lingjianTypes===item.codeIndex?'tabActive':''"
                                  @tap="lingjianTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <view class="list-box4"
        ...
        <block v-for="(product,index) in list" :key="index">
            <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
                ...
                <image
                        :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"280rpx"}'
                        mode="aspectFill" :src="product.lingjianPhoto?baseUrl+product.lingjianPhoto.split(',')[0]:''">
                </image>
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
                            :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                            class="title">{{product.lingjianName}}
                    </view>
                    <view v-if='product.lingjianNewMoney'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"red","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                          class="title">￥{{product.lingjianNewMoney}}
                    </view>
                    <view v-else-if='product.lingjianValue'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                          class="title">￥{{product.lingjianValue}}
                    </view>
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('lingjian','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('lingjian','删除')" class="cuIcon-delete"
            ...
            <view v-if="index%6==1" @tap="onDetailTap(product)"
                ...
                <image
                        :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"280rpx"}'
                        mode="aspectFill" :src="product.lingjianPhoto?baseUrl+product.lingjianPhoto.split(',')[0]:''">
                </image>
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
            ...
            <view v-if="index%6==3" @tap="onDetailTap(product)" class="list-item"
                ...
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
                            :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                            class="title">{{product.lingjianName}}
                    </view>
                    <view v-if='product.lingjianNewMoney'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"red","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 机电零件类型 字段 字典表查询方法
            async lingjianTypesSelect(){
                let lingjianTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'lingjian_types',
                });
                lingjianTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.lingjianTypesList = lingjianTypes.data.list
            },
            //当前表的 机电零件类型字段类别搜索
			lingjianTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.lingjianTypes != 0) {
                    params.lingjianTypes = this.lingjianTypes
                }
                let _this = this
                let res = await this.$api.list(`lingjian`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('lingjian', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`lingjian`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.lingjianName) {
                    searchForm['lingjianName'] = this.searchForm.lingjianName
                }

                let res = await this.$api.list(`lingjian`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.lingjianContent!=null){
                        item.lingjianContent = item.lingjianContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
