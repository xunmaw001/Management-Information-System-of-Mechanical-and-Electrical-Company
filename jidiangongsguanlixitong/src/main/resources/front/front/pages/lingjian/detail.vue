	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
			...
			<view class="">
				<view class="price ">
					...
					<view class="pricejiner"
						:style="detail.lingjianNewMoney>1000? 'width: 450rpx;':detail.lingjianNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
						<text style='font-size:20rpx'>¥</text>
						<text style="font-weight:bold;font-size:50rpx">{{detail.lingjianNewMoney}}</text>
					</view>
				</view>
				...
				<view class="yinyuewai">
					<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.lingjianName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.lingjianContent"></rich-text>
						</view>
					</view>
				</view>
				<!-- 留言 -->
		...
		<view class="bottom-content cu-bar bg-white tabbar border shop">
			<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
				<view class="" style="display: flex; width: 70%;color: #FFFFFF;align-items: center;">
					<uni-number-box :min="1" v-model="buyNumber" />
					<view @tap="onSubmit()"
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.detail('lingjian', this.lingjianId);
				this.detail = res.data
				this.detail.lingjianContent = this.detail.lingjianContent.replace("<img", "<img  class=\"daxiao\"")
					.replace("src=\"upload/", "src=\"" + this.$base.url + "upload/");
				// 主图
				this.swiperList = this.detail.lingjianPhoto ? this.detail.lingjianPhoto.split(",") : [];
			},
			...
			async baoming() {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('lingjianYuyue', {
