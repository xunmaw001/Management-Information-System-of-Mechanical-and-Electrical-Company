	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"0px","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.shebeiName" type="text" placeholder="机电设备名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('shebei','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('shebei','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='shebeiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in shebeiTypesList" :key="index" :class="shebeiTypes===item.codeIndex?'tabActive':''"
                                  @tap="shebeiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <block v-for="(product,index) in list" :key="index">
            <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
                ...
                <image
                        ...
                        mode="aspectFill" :src="product.shebeiPhoto?baseUrl+product.shebeiPhoto.split(',')[0]:''">
                </image>
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
                            ...
                            class="title">{{product.shebeiName}}
                    </view>
                    <view v-if='product.shebeiNewMoney'
                          ...
                          class="title">￥{{product.shebeiNewMoney}}
                    </view>
                    <view v-else-if='product.shebeiValue'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                          class="title">￥{{product.shebeiValue}}
                    </view>
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('shebei','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('shebei','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 机电设备类型 字段 字典表查询方法
            async shebeiTypesSelect(){
                let shebeiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'shebei_types',
                });
                shebeiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.shebeiTypesList = shebeiTypes.data.list
            },
            //当前表的 机电设备类型字段类别搜索
			shebeiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.shebeiTypes != 0) {
                    params.shebeiTypes = this.shebeiTypes
                }
                let _this = this
                let res = await this.$api.list(`shebei`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shebei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`shebei`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.shebeiName) {
                    searchForm['shebeiName'] = this.searchForm.shebeiName
                }

                let res = await this.$api.list(`shebei`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.shebeiContent!=null){
                        item.shebeiContent = item.shebeiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
