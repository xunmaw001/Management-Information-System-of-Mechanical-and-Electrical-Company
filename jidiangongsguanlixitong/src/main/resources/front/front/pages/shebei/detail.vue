			...
			<view class="">
				<view class="price ">
					...
					<view class="pricejiner"
						:style="detail.shebeiNewMoney>1000? 'width: 450rpx;':detail.shebeiNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
						<text style='font-size:20rpx'>¥</text>
						<text style="font-weight:bold;font-size:50rpx">{{detail.shebeiNewMoney}}</text>
					</view>
				</view>
				...
				<view class="yinyuewai">
					<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.shebeiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.shebeiContent"></rich-text>
						</view>
					</view>
				</view>
				<!-- 留言 -->
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.detail('shebei', this.shebeiId);
				this.detail = res.data
				this.detail.shebeiContent = this.detail.shebeiContent.replace("<img", "<img  class=\"daxiao\"")
					.replace("src=\"upload/", "src=\"" + this.$base.url + "upload/");
				// 主图
				this.swiperList = this.detail.shebeiPhoto ? this.detail.shebeiPhoto.split(",") : [];
			},
			...
			async baoming() {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('shebeiYuyue', {
