	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"0px","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.qingjiaText" type="text" placeholder="理由"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('qingjia','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('qingjia','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.qingjiaName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('qingjia','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                 <view v-show="item.qingjiaUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名编号</text>
                     <text style="width: 70%">
                         <text v-if="item.qingjiaUuidNumber != null">{{item.qingjiaUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.qingjiaText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">理由</text>
                    <view style="width: 70%" v-html="item.qingjiaText"></view>
                </view>
                 <view v-show="item.qingjiaYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名状态</text>
                     <text style="width: 70%">{{item.qingjiaYesnoValue}}</text>
                 </view>
                <view v-show="item.qingjiaYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核回复</text>
                    <view style="width: 70%" v-html="item.qingjiaYesnoText"></view>
                </view>
                <view v-show="item.qingjiaShenheTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核时间</text>
                    <text style="width: 70%">
                        <text v-if="item.qingjiaShenheTime != null">{{item.qingjiaShenheTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
                <view v-show="item.qingjiaTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">预约时间</text>
                    <text style="width: 70%">
                        <text v-if="item.qingjiaTime != null">{{item.qingjiaTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('qingjia','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('qingjia','审核') && item.qingjiaYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`qingjia`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('qingjia', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`qingjia`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.qingjiaUuidNumber) {
                    searchForm['qingjiaUuidNumber'] = this.searchForm.qingjiaUuidNumber
                }
				if (this.searchForm.qingjiaText) {
                    searchForm['qingjiaText'] = this.searchForm.qingjiaText
                }

                let res = await this.$api.page(`qingjia`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
