	...
	<view class="content">
		<form>
			...
			<view v-for="(item,index) in orderGoods " v-bind:key="index" class="cu-form-group">
				<image class="avator" :src="baseUrl+item.lingjianPhoto" mode=""></image>
				<view class="title" style="width: 75%;">
					<view style="margin-top: -50rpx;">{{item.lingjianName}}</view>
					<view>
						x{{item.buyNumber}}
						<text style="margin-left: 30upx;color: red;">￥{{item.lingjianNewMoney}}</text>
					</view>
				</view>
			</view>
			<view @tap="onAddressTap" class="cu-form-group">
				...
				<view>
					<text v-if="lingjianOrderPaymentTypes == 1">原价：￥{{(maxNewMouey).toFixed(2)}}</text>
					<view v-if="lingjianOrderPaymentTypes == 1"></view>
					<text v-if="lingjianOrderPaymentTypes == 1">折扣价：￥{{(maxNewMouey * zhekou).toFixed(2)}}</text>
					<text v-if="lingjianOrderPaymentTypes == 2">{{(maxNewMouey).toFixed(2)}}积分</text>
				</view>
			</view>
		</form>
	...
	export default {
		data() {
			return {
				...
				orderGoods: [], //展示数据
				maxNewMouey: 0, //总价格
				lingjianOrderPaymentTypes: 1, //是什么支付类型
				zhi: [{
						id: 1,
						val: "余额"
					},
		...
		async onLoad(options) {
			...
			if (this.orderGoods.length > 0) {
				for (let i = 0; i < this.orderGoods.length; i++) {
					this.maxNewMouey = this.maxNewMouey + parseFloat(this.orderGoods[i].lingjianNewMoney) * this
		...
		async onShow() {
			...
			let kehudengjiTypesRes = await _this.$api.page("dictionary", {
				dicCode: "kehudengji_types",
				dicName: "会员等级类型",
				codeIndexStart: _this.user.kehudengjiTypes,
				codeIndexEnd: _this.user.kehudengjiTypes,
			})
			if (kehudengjiTypesRes.data.list.length > 0) {
				_this.zhekou = Number(kehudengjiTypesRes.data.list[0].beizhu);
			}
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							let data = {
								lingjians: JSON.stringify(_this.orderGoods),
								yonghuId: _this.user.id,
								lingjianOrderPaymentTypes: _this.lingjianOrderPaymentTypes,
								kehuId: _this.kehuId
							}
							await _this.$api.requestConditionDataGet('lingjianOrder', 'order', null, data);
							_this.$utils.jump('/pages/lingjianOrder/list');
						}
					}
				});
			},
