	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.lingjianOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('lingjianOrder', this.lingjianOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.lingjianOrderPhoto ? this.detail.lingjianOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('lingjianOrderYuyue', {
									yonghuId: _this.user.id,
									lingjianOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/lingjianOrderYuyue/list');
							}
						}
					});
				},
