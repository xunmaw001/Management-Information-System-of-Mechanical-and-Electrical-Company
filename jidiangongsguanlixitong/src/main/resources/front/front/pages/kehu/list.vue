	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"0px","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.kehuName" type="text" placeholder="客户姓名"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('kehu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('kehu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.kehuName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('kehu','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.kehuUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">客户编号</text>
                     <text style="width: 70%">
                         <text v-if="item.kehuUuidNumber != null">{{item.kehuUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.kehuPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">客户手机号</text>
                     <text style="width: 70%">
                         <text v-if="item.kehuPhone != null">{{item.kehuPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.kehuIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">客户身份证号</text>
                     <text style="width: 70%">
                         <text v-if="item.kehuIdNumber != null">{{item.kehuIdNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 ...
                 <view v-show="item.kehuEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">客户邮箱</text>
                     <text style="width: 70%">
                         <text v-if="item.kehuEmail != null">{{item.kehuEmail}}</text>
                         <text v-else></text>
                     </text>
                 </view>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('kehu','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('kehu','审核') && item.kehuYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`kehu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.kehuUuidNumber) {
                    searchForm['kehuUuidNumber'] = this.searchForm.kehuUuidNumber
                }
				if (this.searchForm.kehuName) {
                    searchForm['kehuName'] = this.searchForm.kehuName
                }

                let res = await this.$api.list(`kehu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
