	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('shebeiOrder', this.shebeiOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.shebeiOrderPhoto ? this.detail.shebeiOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shebeiOrderYuyue', {
									yonghuId: _this.user.id,
									shebeiOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/shebeiOrderYuyue/list');
							}
						}
					});
				},
